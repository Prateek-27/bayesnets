...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # Get all CPTS with evid
        fact_lst = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        #print(fact_lst)

        for ele in eliminationOrder:
            
            fact_lst, fact_updated = joinFactorsByVariable(fact_lst, ele)

            l = len(fact_updated.unconditionedVariables())

            # If length of uncond ele of updated fact is equal to 2 or more
            if l >= 2:
                fact_nxt = eliminate(fact_updated, ele)
                fact_lst.append(fact_nxt)

        # The ultimate factor we then normalize 
        fact_ultimate = joinFactors(fact_lst)
        #print(fact_ultimate)
        ret = normalize(fact_ultimate)
