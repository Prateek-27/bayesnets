...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        fact_comb = Factor(uncond, cond, dom)
        rng = factor.variableDomainsDict()[eliminationVariable]
...
def normalize(factor):
    ...
    # Assigning Probabilities
    for assig in fact_comb.getAllPossibleAssignmentDicts():
        p = factor.getProbability(assig)
        new_p = p / sum_p
        fact_comb.setProbability(assig, new_p)

    # Probability sum non zero, if zero return none
    if sum_p == 0:
        return None
    else:
        return fact_comb
