...
def constructBayesNet(gameState):
    ...
    # Using above mentioned method to fill out obsVars
    # (Got full set of observation variables)
    
    for posHouse in gameState.getPossibleHouses():
        for wall in gameState.getHouseWalls(posHouse):
...
def fillYCPT(bayesNet, gameState):
    ...
    up = {Y_POS_VAR: BOTH_TOP_VAL}
    leftup = {Y_POS_VAR: LEFT_TOP_VAL}
    down = {Y_POS_VAR: BOTH_BOTTOM_VAL}
    leftdown = {Y_POS_VAR: LEFT_BOTTOM_VAL}

    pos_prob = [up, leftup, down, leftdown]
    corr_prob = [PROB_BOTH_TOP, PROB_ONLY_LEFT_TOP, PROB_BOTH_BOTTOM, PROB_ONLY_LEFT_BOTTOM]
    
    
    # Fill CPT
    
    for i in range(len(pos_prob)):
        yFactor.setProbability(pos_prob[i], corr_prob[i])
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    from inference import inferenceByVariableElimination as infVE
    # Keep track of current assignment and corr. probability 
    curr_assig = 0
    prob_assig = {'curr_assig':[curr_assig, 0]}

    #print(prob_assig, prob_assig['curr_assig'])
    
    fact = infVE(bayesNet, HOUSE_VARS[0], evidence, eliminationOrder)
    
    for assig in fact.getAllPossibleAssignmentDicts():
        p = fact.getProbability(assig)
        
        if p > prob_assig['curr_assig'][1]:
            new_assig = assig
            prob_assig['curr_assig'] = [new_assig, p]
            
    return prob_assig['curr_assig'][0]
